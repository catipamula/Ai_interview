...
function keywordMatchScore(answer, rubric) {
...
const scoreSession = async (sessionId) => {
    ...
    for (const answer of session.answers) {
        ...
        await db_1.default.answer.update({
            where: { id: answer.id },
    ...
    const proctoringViolations = session.proctorEvents.filter(e => ['NO_FACE', 'MULTIPLE_FACES', 'RESTRICTED_OBJECT', 'FACE_MISMATCH'].includes(e.event_type));
    let result = null;
    let rejectionReason = null;
    let rejectionProofs = null;
    if (cheatingDetected) {
        result = 'rejected';
...
exports.scoreSession = scoreSession;
